...
impl DrmSurface {
    ...
    ///
    /// See [`AtomicDrmSurface::use_crtc_color_state`](atomic::AtomicDrmSurface::use_crtc_color_state) for details.
    pub fn use_crtc_color_state(&self, state: CrtcColorState) -> Result<(), Error> {
        match &*self.internal {
            DrmSurfaceInternal::Atomic(surf) => surf.use_crtc_color_state(state),
            DrmSurfaceInternal::Legacy(_) => {
                if state == CrtcColorState::default() {
                    Ok(())
                } else {
                    Err(Error::UnknownProperty {
                        handle: self.crtc.into(),
    ...
    pub fn pending_hdr_hardware_offload(&self) -> bool {
        match &*self.internal {
            DrmSurfaceInternal::Atomic(surf) => surf.pending_hdr_hardware_offload(),
            DrmSurfaceInternal::Legacy(_) => false,
        }
    }
    ...
    pub fn current_hdr_hardware_offload(&self) -> bool {
        match &*self.internal {
            DrmSurfaceInternal::Atomic(surf) => surf.current_hdr_hardware_offload(),
            DrmSurfaceInternal::Legacy(_) => false,
        }
    }
    ...
    pub fn hdr_hardware_offload(&self) -> bool {
        self.pending_hdr_hardware_offload()
    }

    /// Returns the [`CrtcColorState`] to be used after the next commit.
    pub fn pending_crtc_color_state(&self) -> CrtcColorState {
        match &*self.internal {
            DrmSurfaceInternal::Atomic(surf) => surf.pending_crtc_color_state(),
            DrmSurfaceInternal::Legacy(_) => CrtcColorState::default(),
        }
    }
    ...
    pub fn current_crtc_color_state(&self) -> CrtcColorState {
        match &*self.internal {
            DrmSurfaceInternal::Atomic(surf) => surf.current_crtc_color_state(),
            DrmSurfaceInternal::Legacy(_) => CrtcColorState::default(),
        }
    }
