...
impl ColorManagementState {
    ...
    pub fn preferred_changed(&self, surface: &WlSurface, desc: ImageDescription) {
        compositor::with_states(surface, |states| {
            self.preferred_changed_from_states(states, desc);
        });
    }

    /// Variant of [`preferred_changed`](Self::preferred_changed) for use where the surface
    /// state is already borrowed, e.g. inside surface-tree traversal callbacks where re-locking
    /// the surface would deadlock.
    pub fn preferred_changed_from_states(&self, states: &compositor::SurfaceData, desc: ImageDescription) {
    ...
    fn request(
        ...
        match request {
            ...
            Request::GetSurface { id, surface } => {
                let (already_attached, is_initial) = compositor::with_states(&surface, |states| {
                    states
                        .data_map
                    ...
                    let already_attached = std::mem::replace(&mut *attached, true);
                    let mut hook_installed = data.hook_installed.lock().unwrap();
                    let is_initial = !*hook_installed;
                    *hook_installed = true;
                    (already_attached, is_initial)
                });
                if already_attached {
                    resource.post_error(
                ...
                // Make sure we do not install the hook more than once in case the surface is being reused
                if is_initial {
                    compositor::add_post_commit_hook::<D, _>(&surface, |_, _, surface| {
                        compositor::with_states(surface, |states| {
                            if let Some(data) = states
                                ...
                                data.lock().unwrap().update_color_state(states);
                            }
                        });
                    });
                }
